...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        if eth.ether_types == ether_types.ETH_TYPE_ARP:
            self.logger.info("This is an ARP packet")
            arph = pkt.get_protocol(arp.arp)
            arprq = arph.opcode
            self.logger.info("ARP operation code %d", arprq)
            if arph == 1:
    ...
    def _packet_in_handler(self, ev):
        ...
        if len(arph) != 0:
            ...
            self.logger.info("This packet is not an ARP")'''
    ...
    # added to avoid ARP messages
        #if arprq == 1 && dst == 'ff:ff:ff:ff:ff:ff':
        #    match = parser.OFPMatch(eth_dst='ff:ff:ff:ff:ff:ff', arp_op='1')
        #    actions = [parser.OFPActionOutput(ofproto.OFPP_FLOOD)] 
        #    self.add_flow(datapath, 0, match, actions)
        #    return
